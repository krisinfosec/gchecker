...
def msg_usage()
   puts "usage: " + $FILENAME + "\"keyword\" \"domain\""
end
if (ARGV.length < 2) 
  msg_usage()
  exit   
...
while found == false and page_num < max_pages
  ...
  elements = Hpricot.parse(
      open(
"http://google.com/search?q=#{query}&start=#{result_num}&sa=N&hl=en",'Proxy'=>proxy_used,'User-Agent'=>'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; pl-PL; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2')
).search("li.g h3.r a")
  elements.each do |el|
    position += 1
